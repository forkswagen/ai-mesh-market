...
export function formatOrchestratorHttpError(path: string, status: number, body: string): string {
  ...
  if (looksHtml || status === 404) {
    const base =
      ...
      "(задайте VITE_API_BASE_URL, либо api/escora + env Functions, либо ORCHESTRATOR_UPSTREAM_URL + VITE_ORCHESTRATOR_VIA_PROXY=1)";
    ...
    return (
      ...
      `3) Монолит Vercel: без отдельного VITE_API_BASE_URL база — этот же origin; проверьте rewrites и env Functions. Отдельный server/: VITE_API_BASE_URL на его URL.\n\n` +
