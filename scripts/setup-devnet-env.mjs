#!/usr/bin/env node
/**
 * Generates server/.env with three devnet keypairs and requests airdrop.
 * From repo root: node scripts/setup-devnet-env.mjs
 * Overwrite existing: node scripts/setup-devnet-env.mjs --force
 */
...
if (existsSync(envPath) && !force) {
  console.error(`Already exists ${envPath}. To recreate: node scripts/setup-devnet-env.mjs --force`);
  process.exit(1);
}
...
console.log("Requesting devnet airdrop (may take 10–20 s)…");
for (const [label, kp] of [
  ...
  try {
    ...
    console.warn(`⚠ airdrop ${label} failed: ${e.message}`);
    console.warn("  Fund manually: solana airdrop 2 <PUBKEY> --url devnet");
    console.warn(`  ${label} pubkey: ${kp.publicKey.toBase58()}`);
  }
}

const env = `# Generated by scripts/setup-devnet-env.mjs — local devnet demo only
SOLANA_RPC_URL=${RPC}
...
# LM Studio (uncomment for LLM oracle on demo writes)
...
console.log(`\n✓ Wrote ${envPath}`);
console.log("Restart stack: npm run dev from root (or API only: npm run server:dev)");
