...
if (!existsSync(envPath)) {
  console.error("❌ Нет server/.env — npm run setup:devnet");
  ok = false;
} else {
  ...
  for (const key of ["BUYER_SECRET_JSON", "SELLER_SECRET_JSON", "ORACLE_SECRET_JSON"]) {
    ...
    if (!m || m[1].length < 100) {
      console.error(`❌ Некорректный ${key} в server/.env`);
      ok = false;
    }
  }
  if (ok) {
    ...
    for (const [label, json] of [
      ...
      if (lamports < minLamports) {
        console.error(
          `❌ ${label} ${kp.publicKey.toBase58()} на devnet: ${(lamports / 1e9).toFixed(4)} SOL (нужно ≥ ${minLamports / 1e9} SOL; faucet https://faucet.solana.com )`,
        );
        ok = false;
      } else {
...
try {
  ...
  console.error(`❌ Оркестратор ${base}:`, e.message);
  ok = false;
}
